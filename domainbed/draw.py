...
def save_all_layer_mask_scatter_plots(domain_importance, save_dir, k_percentage=0.1, title_prefix="Mask Scatter Plot", figsize=(10, 6)):
    """
    ...
    for layer_name in all_layers:
        ...
        plt.scatter(
            ...
            color="#4E79A7", alpha=1.0, s=20
        )

        # 添加标题和坐标轴
        plt.title(f"Importrance mask in PACS",fontsize=24)
        plt.xlabel("Input Dim",fontsize=20)
        plt.ylabel("Output Dim",fontsize=20)
       #  plt.legend()

        # 保存图像到文件
        save_path = f"{save_dir}/{layer_name}_all_scatter_plot.png"
